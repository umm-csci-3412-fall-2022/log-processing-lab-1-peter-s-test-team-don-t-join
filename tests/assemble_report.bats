...
setup() {
  ...
  cp tests/username_dist.html "$BATS_TMPDIR"/data
  cp tests/hours_dist.html "$BATS_TMPDIR"/data
  cp tests/country_dist.html "$BATS_TMPDIR"/data
...
@test "bin/assemble_report.sh runs successfully" {
  run bin/assemble_report.sh data
  assert_success
}
...
@test "bin/assemble_report.sh generates correct simple output" {
  run bin/assemble_report.sh data
  run diff -wbB tests/failed_login_summary.html data/failed_login_summary.html
  assert_success
}
